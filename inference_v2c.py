...
config = yaml.safe_load(open("output/{}/config_v2c.yml".format(args.exp_name)))
...
def get_visual_feature(speaker, name):
    ...
    atm_path = os.path.join(
            data_params['feature_root_path'],
            "emos",
            "{}-emo-{}.npy".format(speaker, name),
        )
    atm_feature = torch.from_numpy(np.load(atm_path)).float().to(device)

    return lip_feature, emotion_feature, atm_feature
...
def inference(text, ref_s, alpha = 0.0, beta = 0.3, diffusion_steps=5, embedding_scale=1, 
    ...
    with torch.no_grad():
        ...
        pred_dur[0] += (duration_sum / 2) - pred_dur.sum()
...
if args.setting == 1:
    ...
    for i in tqdm(range(len(name))):
        ...
        lip_feature, emotion_feature, atm_feature = get_visual_feature(speaker[i], name[i])
        ...
        wav = inference(raw_text[i], ref_s, embedding_scale=1, 
                        mel_len=mel_len, emotion_feature=emotion_feature, lip_feature=lip_feature, atm_feature=atm_feature)
...
if args.setting == 3:
    ...
    wav_path = config['val_wav_path_setting3']
    text_path = config['val_wav_path']
    ...
    with open(val_path, "r", encoding="utf-8") as f:
            ...
            raw_text = []
            for line in f.readlines():
                n, p, rn, rt = line.strip("\n").split("|")
                name.append(n)
                ref_name.append(rn)
                raw_text_path = "{}/{}/{}.lab".format(text_path, n.split('_00')[0], n)
                raw_text.append(read_lab_file(raw_text_path)[0])

    noise = torch.randn(1,1,256).to(device)
    for i in tqdm(range(len(name))):

        lip_feature, emotion_feature, atm_feature = get_visual_feature(name[i].split('_00')[0], name[i])
        gt_wav_path = "{}/{}/{}.wav".format(text_path, name[i].split('_00')[0], name[i])
                
        try:
            ref_s, _ = compute_style("{}/{}/{}.wav".format(wav_path, ref_name[i].split('-')[0], ref_name[i]))
            _, mel_len = compute_style(gt_wav_path, return_mel_len=True)
            wav = inference(raw_text[i], ref_s, embedding_scale=1, 
                            mel_len=mel_len, emotion_feature=emotion_feature, lip_feature=lip_feature,
                            atm_feature=atm_feature)
            sf.write('{}/wav_pred_{}_{}.wav'.format(save_path, name[i], ref_name[i]), wav, 24000)
        except Exception:
            continue
